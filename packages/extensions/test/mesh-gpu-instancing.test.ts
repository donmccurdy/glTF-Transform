...
test('@gltf-transform/extensions::mesh-gpu-instancing', t => {
	...
	const data = doc.createAccessor('unused')
	...
	const prim = doc.createPrimitive()
		.setAttribute('POSITION', data.clone().setName('prim_pos'));
	...
	t.deepEqual(nodeDef.extensions, {'EXT_mesh_gpu_instancing': {
...
test('@gltf-transform/extensions::mesh-gpu-instancing | copy', t => {
	const doc = new Document();
	const data = doc.createAccessor()
		.setArray(new Float32Array(12))
		.setType(Accessor.Type.VEC3)
		.setBuffer(doc.createBuffer());
	const batchExtension = doc.createExtension(MeshGPUInstancing);
	const batch = batchExtension.createInstancedMesh()
		.setAttribute('TRANSLATION', data)
		.setAttribute('_ID', data);

	doc.createNode().setExtension('EXT_mesh_gpu_instancing', batch);

	const doc2 = doc.clone();
	const batch2 = doc2.getRoot().listNodes()[0]
		.getExtension<InstancedMesh>('EXT_mesh_gpu_instancing');
