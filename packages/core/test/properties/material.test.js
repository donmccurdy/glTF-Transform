...
import { Document, Format, NodeIO, TextureChannel, TextureInfo, TextureLink } from '../../';
...
test('@gltf-transform/core::material | texture channels', t => {
    ...
    function getChannels(texture) {
        let mask = 0x0000;
        for (const link of graph.listParentLinks(texture)) {
            if (link instanceof TextureLink)
                mask |= link.channels;
        }
        return mask;
    }
...
test('@gltf-transform/core::material | i/o', t => {
    const doc = new Document();
    doc.createBuffer();
    const createTexture = (name) => doc.createTexture(name)
